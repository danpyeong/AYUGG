  ...
  for (let i = 0; i < textList.length; i++) {
    ...
    rawResult = await fetch(UserUrl)
      ...
      .then(async (rawData) => {
        ...
        let example1 = await fetch(UidUrl)
          .then((response1) => {
        ...
        let example2 = await fetch(MatchUrl)
          .then((response2) => {
...
function createBox(i, div) {
  ...
  lossesDiv.style.width =
    (dataList[i].losses / (dataList[i].wins + dataList[i].losses)) * 130 -
    7 +
    "px";
  ...
  rateDiv.textContent =
    Math.floor(
      (dataList[i].wins / (dataList[i].wins + dataList[i].losses)) * 100
    ) + "%";
...
function matchBoxCreate(i, j, div) {
  ...
  champImg.src =
    version +
    "img/champion/" +
    JSON.parse(JSON.stringify(dataList[i])).matches[j].championName +
    ".png";
  ...
  KDADiv.textContent =
    dataList[i].matches[j].kills +
    "/" +
    dataList[i].matches[j].deaths +
    "/" +
    dataList[i].matches[j].assists;

  const timeDiv = document.createElement("div");
  timeDiv.style.margin = "auto 0 auto auto";
  if (
    Math.floor(
      (eta.getTime() - dataList[i].matches[j].gameStartTimestamp) / 3600000
    ) < 24
  ) {
    timeDiv.textContent =
      Math.floor(
    ...
    timeDiv.textContent =
      Math.floor(
        (eta.getTime() - dataList[i].matches[j].gameStartTimestamp) /
          3600000 /
          24
      ) + "일전";
  }
