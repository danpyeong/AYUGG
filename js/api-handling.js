          ...
          for (let j = 0; j < 2; j++) {
            ...
            for (let i = 0; i < 10; i++) {
              if (dataSet.get("id") == appData.participants[i].summonerId) {
                ...
                let UidguestUrl =
                  TierUrl +
                  appData.participants[i].summonerId +
                  "?api_key=" +
                  apiKey;
                var example3 = fetch(UidguestUrl)
                  .then((response2) => {
                    return response2.json();
                  })
                  .then((rawData2) => {
...
setTimeout(function () {
  ...
  sessionStorage.setItem(
    "tier",
    dataSet.get("tier") + " " + dataSet.get("rank")
  );
  ...
  statitext1.innerHTML =
    Math.floor(
      (dataSet.get("wins") / (dataSet.get("wins") + dataSet.get("losses"))) *
        100
    ) + "%";
  var statitext2 = document.getElementById("statiWL");
  statitext2.innerHTML =
    dataSet.get("wins") + "승 " + dataSet.get("losses") + "패";
  ...
  statiKDAtext.innerText =
    "KDA " +
    (
      (statiMostWhole.get("kills") + statiMostWhole.get("assists")) /
      statiMostWhole.get("deaths")
    ).toFixed(2);
  var statiKda = document.getElementById("statiKda");
  statiKda.innerText =
    (statiMostWhole.get("kills") / statiMostWhole.get("games")).toFixed(1) +
    "/" +
    (statiMostWhole.get("deaths") / statiMostWhole.get("games")).toFixed(1) +
    "/" +
    (statiMostWhole.get("assists") / statiMostWhole.get("games")).toFixed(1);
  var statiKillRate = document.getElementById("statiKillRate");
  statiKillRate.innerText =
    "킬관여 " +
    Math.floor(
      ((statiMostWhole.get("kills") + statiMostWhole.get("assists")) /
        statiMostWhole.get("killsTeam")) *
        100
    ) +
    "%";
  ...
  SMWtext3.innerText =
    "CS " + Math.floor(statiMostWhole.get("CS") / statiMostWhole.get("games"));
  var SMWtext4 = document.getElementById("SMWRate");
  SMWtext4.innerText =
    Math.floor(
      (statiMostWhole.get("win") / statiMostWhole.get("games")) * 100
    ) + "%";
  ...
  document.getElementById("SMChampIcon").src =
    version + "img/champion/" + statiMost[0].get("champname") + ".png";
  var SMtext1 = document.getElementById("SMKDARate");
  SMtext1.innerText =
    "KDA " +
    (
      (statiMost[0].get("kills") + statiMost[0].get("assists")) /
      statiMost[0].get("deaths")
    ).toFixed(2);
  var SMtext2 = document.getElementById("SMKDA");
  SMtext2.innerText =
    (statiMost[0].get("kills") / statiMost[0].get("games")).toFixed(1) +
    "/" +
    (statiMost[0].get("deaths") / statiMost[0].get("games")).toFixed(1) +
    "/" +
    (statiMost[0].get("assists") / statiMost[0].get("games")).toFixed(1);
  var SMtext3 = document.getElementById("SMCS");
  SMtext3.innerText =
    "CS " + Math.floor(statiMost[0].get("CS") / statiMost[0].get("games"));
  var SMtext4 = document.getElementById("SMRate");
  SMtext4.innerText =
    Math.floor((statiMost[0].get("win") / statiMost[0].get("games")) * 100) +
    "%";
  ...
  document.getElementById("SMChampIcon1").src =
    version + "img/champion/" + statiMost[1].get("champname") + ".png";
  var SMtext11 = document.getElementById("SMKDARate1");
  SMtext11.innerText =
    "KDA " +
    (
      (statiMost[1].get("kills") + statiMost[1].get("assists")) /
      statiMost[1].get("deaths")
    ).toFixed(2);
  var SMtext21 = document.getElementById("SMKDA1");
  SMtext21.innerText =
    (statiMost[1].get("kills") / statiMost[1].get("games")).toFixed(1) +
    "/" +
    (statiMost[1].get("deaths") / statiMost[1].get("games")).toFixed(1) +
    "/" +
    (statiMost[1].get("assists") / statiMost[1].get("games")).toFixed(1);
  var SMtext31 = document.getElementById("SMCS1");
  SMtext31.innerText =
    "CS " + Math.floor(statiMost[1].get("CS") / statiMost[1].get("games"));
  var SMtext41 = document.getElementById("SMRate1");
  SMtext41.innerText =
    Math.floor((statiMost[1].get("win") / statiMost[1].get("games")) * 100) +
    "%";
  ...
  document.getElementById("SMChampIcon2").src =
    version + "img/champion/" + statiMost[2].get("champname") + ".png";
  var SMtext12 = document.getElementById("SMKDARate2");
  SMtext12.innerText =
    "KDA " +
    (
      (statiMost[2].get("kills") + statiMost[2].get("assists")) /
      statiMost[2].get("deaths")
    ).toFixed(2);
  var SMtext22 = document.getElementById("SMKDA2");
  SMtext22.innerText =
    (statiMost[2].get("kills") / statiMost[2].get("games")).toFixed(1) +
    "/" +
    (statiMost[2].get("deaths") / statiMost[2].get("games")).toFixed(1) +
    "/" +
    (statiMost[2].get("assists") / statiMost[2].get("games")).toFixed(1);
  var SMtext32 = document.getElementById("SMCS2");
  SMtext32.innerText =
    "CS " + Math.floor(statiMost[2].get("CS") / statiMost[2].get("games"));
  var SMtext42 = document.getElementById("SMRate2");
  SMtext42.innerText =
    Math.floor((statiMost[2].get("win") / statiMost[2].get("games")) * 100) +
    "%";
  ...
  document.getElementById("player-icon").src =
    version + "img/profileicon/" + dataSet.get("icon") + ".png";
  ...
  if (dataSet.has("tier")) {
    ...
    document.getElementById("tier-icon").src =
      "/images/tier-icons/provisional.png";
  ...
  for (let k = 0; k < 2; k++) {
    ...
    if (
      Math.floor(
        (eta.getTime() - match[k].get("matchData").gameStartTimestamp) / 3600000
      ) < 24
    ) {
      matchDateDiv.textContent =
        Math.floor(
          (eta.getTime() - match[k].get("matchData").gameStartTimestamp) /
            3600000
        ) + "시간전";
    } else {
      matchDateDiv.textContent =
    ...
    matchTimeDiv.textContent =
      Math.floor(match[k].get("matchData").gameDuration / 60) +
      ":" +
      (match[k].get("matchData").gameDuration % 60).toString().padStart(2, "0");
    ...
    const towerBlueSvg = document.createElementNS(
    ...
    const towerBluePath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    towerBluePath.style.width = "20px";
    towerBluePath.style.height = "20px";
    towerBluePath.setAttribute(
      "d",
      "M13.635 2.5V4.58333H11.1825V2.5H8.81752V4.58333H6.36496V2.5H4V8.75L5.83942 9.25V14.9167L4 15.4167V17.5H16V15.4167L14.073 14.9167V9.25L16 8.75V2.5H13.635ZM11.1825 13.9167H8.81752V10L9.9562 9.16667L11.0949 10V13.9167H11.1825Z"
    );
    ...
    const dragonBlueSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const baronBlueSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const towerRedSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const towerRedPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    towerRedPath.setAttribute(
      "d",
      "M13.635 2.5V4.58333H11.1825V2.5H8.81752V4.58333H6.36496V2.5H4V8.75L5.83942 9.25V14.9167L4 15.4167V17.5H16V15.4167L14.073 14.9167V9.25L16 8.75V2.5H13.635ZM11.1825 13.9167H8.81752V10L9.9562 9.16667L11.0949 10V13.9167H11.1825Z"
    );
    ...
    const dragonRedSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const dragonRedPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    dragonRedPath.setAttribute(
      "d",
      "M17.8697 7.08361L15.5522 7.42609C15.4132 7.42609 15.3205 7.38328 15.3205 7.25485L15.1351 3.61605C15.1351 3.48763 15.0424 3.48763 14.9497 3.57324L12.0297 6.27023C11.937 6.35585 11.8443 6.31304 11.7979 6.22742L10.1294 2.28896C10.083 2.16054 10.0367 2.07492 10.0367 2.03211C10.0367 1.9893 10.0367 1.9893 10.0367 2.03211C10.0367 2.03211 9.99032 2.16054 9.94398 2.28896L8.2754 6.22742C8.22905 6.35585 8.13635 6.35585 8.04366 6.27023L5.03096 3.57324C4.93826 3.48763 4.84556 3.53043 4.84556 3.65886L4.66016 7.29766C4.66016 7.42609 4.56746 7.51171 4.42842 7.4689L2.11095 7.12642C1.97191 7.12642 1.97191 7.16923 2.06461 7.25485L4.52111 9.39532C4.61381 9.48094 4.70651 9.65217 4.70651 9.73779L5.12365 12.6916C5.17 13.1625 5.4481 13.5906 5.86524 13.8475C6.60683 14.3612 9.38778 17.8288 9.38778 17.8288C9.48048 17.9144 9.61953 18 9.75858 18C9.89763 18 9.99032 18 9.99032 18C9.99032 18 10.1294 18 10.2221 18C10.3148 18 10.5002 17.9144 10.5929 17.8288C10.5929 17.8288 13.3275 14.3184 14.1154 13.8475C14.5325 13.5906 14.8106 13.1625 14.857 12.6916L15.2741 9.73779C15.2741 9.60936 15.3668 9.43813 15.4595 9.39532L17.916 7.25485C18.0551 7.12642 18.0087 7.08361 17.8697 7.08361ZM8.59985 12.4348C6.74588 11.5786 6.28239 9.43813 6.28239 9.43813L9.06334 11.5786C9.06334 11.5786 9.52683 12.4348 8.59985 12.4348ZM11.3808 12.4348C10.4538 12.4348 10.9173 11.5786 10.9173 11.5786L13.6983 9.43813C13.6983 9.43813 13.2348 11.5786 11.3808 12.4348Z"
    );
    ...
    const baronRedSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const baronRedPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    baronRedPath.setAttribute(
      "d",
      "M15.0357 8.73753L17 6.16463C17 6.16463 15.5848 4.08573 15.5543 4.01711C15.5543 4.01711 15.5542 4.01711 15.5481 4.01025L12.1625 1.99996C12.1564 1.99996 12.1442 2.00682 12.1503 2.01368C12.1503 2.01368 14.139 4.86789 14.139 4.87475C14.139 4.87475 14.139 6.14404 14.139 6.15091L12.3028 7.02912L10.0091 4.89533L7.70937 7.02912L5.861 6.15777C5.861 6.15091 5.861 4.88161 5.861 4.88161L7.84967 2.02055C7.85577 2.01368 7.84968 1.99996 7.83748 2.00682L4.45185 4.01711C4.45185 4.01711 4.45185 4.01711 4.44575 4.02398C4.41525 4.09259 3 6.17149 3 6.17149L4.96427 8.74439C4.96427 8.74439 4.57995 9.80099 4.44575 10.0823L3.00609 11.4476L6.44662 16.9091L8.61829 18L6.77603 14.6655L7.64226 14.8096L8.6427 15.9554V15.1663L9.04531 15.2075L9.99695 16.4356C9.99695 16.4356 10.9425 15.2075 10.9486 15.2075L11.3512 15.1663V15.9554L12.3516 14.8096L13.2179 14.6655L11.3756 18L13.5473 16.9091L16.9878 11.4476L15.5481 10.0823C15.4261 9.79413 15.0357 8.73753 15.0357 8.73753ZM7.45316 12.2641C6.98954 12.2641 6.61743 11.8387 6.61743 11.3241C6.61743 10.8027 6.98954 10.3842 7.45316 10.3842C7.91678 10.3842 8.28889 10.8027 8.28889 11.3241C8.28889 11.8456 7.91678 12.2641 7.45316 12.2641ZM10.0091 13.7667C9.55163 13.7598 9.18562 13.3413 9.18562 12.8267C9.18562 12.3121 9.55163 11.8936 10.0091 11.8868C10.4667 11.8936 10.8327 12.3121 10.8327 12.8267C10.8327 13.3482 10.4606 13.7598 10.0091 13.7667ZM10.0091 10.3293C9.46623 10.3293 9.02702 9.8353 9.02702 9.22466C9.02702 8.61403 9.46623 8.12003 10.0091 8.12003C10.5521 8.12003 10.9913 8.61403 10.9913 9.22466C10.9852 9.8353 10.546 10.3293 10.0091 10.3293ZM12.559 12.2641C12.0954 12.2641 11.7233 11.8456 11.7233 11.3241C11.7233 10.8027 12.0954 10.3842 12.559 10.3842C13.0227 10.3842 13.3948 10.8027 13.3948 11.3241C13.3948 11.8387 13.0227 12.2641 12.559 12.2641Z"
    );
    ...
    for (let finduser = 0; finduser < 10; finduser++) {
      if (
        dataSet.get("id") ==
        match[k].get("matchData").participants[finduser].summonerId
      ) {
        ...
        champIconImg.src =
          version + "img/champion/" + owner.championName + ".png";
        ...
        fetch(spellUrl)
          ...
          .then((data) => {
            ...
            spell1Img.src =
              version + "img/spell/" + findSpellKey(owner.summoner1Id);
            spell2Img.src =
              version + "img/spell/" + findSpellKey(owner.summoner2Id);
          });
        ...
        fetch(runeUrl)
          ...
          .then((data) => {
            ...
            summoner1IdImg.src =
              ...
              findMainRuneKey(
                owner.perks.styles[0].style,
                owner.perks.styles[0].selections[0].perk
              );
            summoner2IdImg.src =
              "https://ddragon.leagueoflegends.com/cdn/img/" +
              findRuneKey(owner.perks.styles[1].style);
          });
        kdaDiv.textContent =
          owner.kills + "/" + owner.deaths + "/" + owner.assists;
        avgKDADiv.textContent = (
          (owner.kills + owner.assists) /
          owner.deaths
        ).toFixed(1);
        if (owner.teamId == 100) {
          killRateSpan.textContent =
            "킬관여 " +
            Math.floor(
              ((owner.kills + owner.assists) /
                match[k].get("matchData").teams[0].objectives.champion.kills) *
                100
            ) +
            "%  ";
        } else {
          killRateSpan.textContent =
            ...
            Math.floor(
              ((owner.kills + owner.assists) /
                match[k].get("matchData").teams[1].objectives.champion.kills) *
                100
            ) +
            "%  ";
        }
        CSSpan.textContent =
          "CS " + (owner.totalMinionsKilled + owner.neutralMinionsKilled);
        if (owner.item0 != 0) {
    ...
    text14.innerText =
      match[k].get("matchData").teams[0].objectives.tower.kills;
    text15.innerText =
      match[k].get("matchData").teams[0].objectives.dragon.kills;
    text16.innerText =
      match[k].get("matchData").teams[0].objectives.baron.kills;
    text17.innerText =
      match[k].get("matchData").teams[1].objectives.tower.kills;
    text18.innerText =
      match[k].get("matchData").teams[1].objectives.dragon.kills;
    text19.innerText =
      match[k].get("matchData").teams[1].objectives.baron.kills;
...
function partiListMaking1(ul, k) {
  for (let i = 0; i < 5; i++) {
    ...
    childImg.src =
      version +
      "img/champion/" +
      match[k].get("matchData").participants[i].championName +
      ".png";
    childSpan2.textContent =
      match[k].get("matchData").participants[i].summonerName;
...
function matchDtFirstBlue(Id, k) {
  for (let i = 0; i < 5; i++) {
    ...
    childImg0.src =
      ...
      match[k].get("matchData").participants[i].championName +
      ".png";
    childDiv1.textContent =
      match[k].get("matchData").participants[i].champLevel;
    ...
    fetch(spellUrl)
      ...
      .then((data) => {
        ...
        childImg1.src =
          version +
          "img/spell/" +
          findSpellKey(match[k].get("matchData").participants[i].summoner1Id);
        childImg1.classList.add("dt-small-img");
        childImg1.style.paddingRight = "3px";
        childImg2.src =
          version +
          "img/spell/" +
          findSpellKey(match[k].get("matchData").participants[i].summoner2Id);
    ...
    fetch(runeUrl)
      ...
      .then((data) => {
        ...
        childImg3.src =
          "https://ddragon.leagueoflegends.com/cdn/img/" +
          findMainRuneKey(
            match[k].get("matchData").participants[i].perks.styles[0].style,
            match[k].get("matchData").participants[i].perks.styles[0]
              .selections[0].perk
          );
        ...
        childImg4.src =
          "https://ddragon.leagueoflegends.com/cdn/img/" +
          findRuneKey(
            match[k].get("matchData").participants[i].perks.styles[1].style
          );
        childImg4.classList.add("dt-small-img");
      });
    // console.log(match);
    childDiv4.classList.add("nick-tier");
    childDiv5.textContent =
      match[k].get("matchData").participants[i].summonerName;
    childDiv6.textContent = match[k].get("matchUser")[i];
    childDiv6.classList.add("font3");
    childDiv7.textContent =
      match[k].get("matchData").participants[i].totalDamageDealtToChampions;
    childDiv15.classList.add("damage-graph");
    childDiv16.classList.add("bar");
    childDiv16.style.width =
      (match[k].get("matchData").participants[i].totalDamageDealtToChampions /
        mostDamage(k)) *
        60 +
      "px";
    childDiv9.textContent =
      match[k].get("matchData").participants[i].kills +
      "/" +
      match[k].get("matchData").participants[i].deaths +
      "/" +
      match[k].get("matchData").participants[i].assists;
    childDiv10.textContent =
      "(" +
      (
        (match[k].get("matchData").participants[i].kills +
          match[k].get("matchData").participants[i].assists) /
    ...
    childDiv11.textContent =
      match[k].get("matchData").participants[i].totalMinionsKilled +
      match[k].get("matchData").participants[i].neutralMinionsKilled;
    childDiv12.textContent =
      "(" +
      (
        ((match[k].get("matchData").participants[i].totalMinionsKilled +
          match[k].get("matchData").participants[i].neutralMinionsKilled) /
          match[k].get("matchData").gameDuration) *
        60
      ).toFixed(1) +
      ")";
    childDiv12.classList.add("font3");
    childDiv13.textContent =
      match[k].get("matchData").participants[i].wardsPlaced;
    childDiv14.textContent =
      "(" +
      (match[k].get("matchData").participants[i].wardsPlaced -
        match[k].get("matchData").participants[i].visionWardsBoughtInGame) +
      "/" +
      match[k].get("matchData").participants[i].visionWardsBoughtInGame +
      ")";
    ...
    if (match[k].get("matchData").participants[i].item0 == 0) {
      ...
      childImg6.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item0 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item1 == 0) {
      ...
      childImg7.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item1 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item2 == 0) {
      ...
      childImg8.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item2 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item3 == 0) {
      ...
      childImg9.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item3 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item4 == 0) {
      ...
      childImg10.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item4 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item5 == 0) {
      ...
      childImg11.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item5 +
        ".png";
    }
    if (match[k].get("matchData").participants[i].item6 == 0) {
      ...
      childImg12.src =
        version +
        "img/item/" +
        match[k].get("matchData").participants[i].item6 +
        ".png";
    }
...
function mostDamage(k) {
  let mostDamage =
    match[k].get("matchData").participants[0].totalDamageDealtToChampions;
  for (let i = 1; i < 10; i++) {
    if (
      mostDamage <
      match[k].get("matchData").participants[i].totalDamageDealtToChampions
...
function statiMostChamp() {
  ...
  for (let i = 0; i < Num; i++) {
    for (let finduser = 0; finduser < 10; finduser++) {
      if (
        dataSet.get("id") ==
        match[i].get("matchData").participants[finduser].summonerId
      ) {
        ...
        statiexSet.set(
          "CS",
          owner.totalMinionsKilled + owner.neutralMinionsKilled
        );
        if (owner.win == true) {
  ...
  for (let b = 0; b < Num - 1; b++) {
    for (let a = b + 1; a < Num; a++) {
      if (statiexMostChamp[a] != null && statiexMostChamp[b] != null) {
        if (
          statiexMostChamp[a].get("champname") ==
          statiexMostChamp[b].get("champname")
        ) {
          statiexMostChamp[b].set(
            "kills",
            statiexMostChamp[a].get("kills") + statiexMostChamp[b].get("kills")
          );
          statiexMostChamp[b].set(
            "deaths",
            statiexMostChamp[a].get("deaths") +
              statiexMostChamp[b].get("deaths")
          );
          statiexMostChamp[b].set(
            "assists",
            statiexMostChamp[a].get("assists") +
              statiexMostChamp[b].get("assists")
          );
          statiexMostChamp[b].set(
            "CS",
            statiexMostChamp[a].get("CS") + statiexMostChamp[b].get("CS")
          );
          statiexMostChamp[b].set(
            "win",
            statiexMostChamp[a].get("win") + statiexMostChamp[b].get("win")
          );
          statiexMostChamp[b].set(
            "games",
            statiexMostChamp[a].get("games") + statiexMostChamp[b].get("games")
          );
...
function statiMostWholeChamp() {
    ...
    for (let finduser = 0; finduser < 10; finduser++) {
      if (
        ...
        statiexSet.set(
          ...
          match[1]
            .get("matchData")
            .participants[3].challenges.damagePerMinute.toFixed(2)
        ); //max: 1500
        statiexSet.set(
          "goldPerMinute",
          match[1]
            .get("matchData")
            .participants[3].challenges.goldPerMinute.toFixed(2)
        ); //max:400
        statiexSet.set(
          "visionScorePerMinute",
          match[1]
            .get("matchData")
            .participants[3].challenges.visionScorePerMinute.toFixed(2)
        ); //max: 1.5
        if (owner.win == true) {
          statiexSet.set("win", 1);
        } else {
        ...
        if (owner.teamId == 100) {
          statiexSet.set(
            "killsTeam",
            match[i].get("matchData").teams[0].objectives.champion.kills
          );
        } else {
          statiexSet.set(
            "killsTeam",
            match[i].get("matchData").teams[1].objectives.champion.kills
          );
        }
        statiexSet.set("games", 1);
  ...
  for (let a = 0; a < Num - 1; a++) {
    statiMostWholeChamp[a + 1].set(
      "kills",
      statiMostWholeChamp[a].get("kills") +
        statiMostWholeChamp[a + 1].get("kills")
    );
    statiMostWholeChamp[a + 1].set(
      "deaths",
      statiMostWholeChamp[a].get("deaths") +
        statiMostWholeChamp[a + 1].get("deaths")
    );
    statiMostWholeChamp[a + 1].set(
      "assists",
      statiMostWholeChamp[a].get("assists") +
        statiMostWholeChamp[a + 1].get("assists")
    );
    statiMostWholeChamp[a + 1].set(
      "CS",
      statiMostWholeChamp[a].get("CS") + statiMostWholeChamp[a + 1].get("CS")
    );
    statiMostWholeChamp[a + 1].set(
      "win",
      statiMostWholeChamp[a].get("win") + statiMostWholeChamp[a + 1].get("win")
    );
    statiMostWholeChamp[a + 1].set(
      "games",
      statiMostWholeChamp[a].get("games") +
        statiMostWholeChamp[a + 1].get("games")
    );
    statiMostWholeChamp[a + 1].set(
      "killsTeam",
      statiMostWholeChamp[a].get("killsTeam") +
        statiMostWholeChamp[a + 1].get("killsTeam")
    );
  }
